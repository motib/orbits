...
\clip[draw] (-5.1,-5.7) rectangle +(10.2,11.5);

% Draw the orbit of the Earth and the celestial sphere
\coordinate (S) at (0,0) node[above right,xshift=-2pt,yshift=-2pt] {$O,S$};
\draw[name path=Eorbit] (S) circle[radius=3cm];
\draw[name path=Csphere] (S) circle[radius=5cm];
...
\node[above left,xshift=0pt] at (SE) {\textsf{SE}};
\coordinate (AE) at (180:5cm);
%\node[left,xshift=-5pt] at (AE) {\textsf{autumn equinox}};
\node[above right,xshift=0pt] at (AE) {\textsf{AE}};
...
\path [name intersections={of=Eorbit and Equi, by={Sp,Au}}];
\path [name intersections={of=Eorbit and Sols, by={Su,Wi}}];

% Names of the seasons
\node at (45:1.8) {\textsf{spring}};
\node at (135:1.8) {\textsf{summer}};
\node at (-135:1.8) {\textsf{autumn}};
\node at (-45:1.8) {\textsf{winter}};

\end{tikzpicture}
\caption{The orbit of the Earth\\and the seasons\\%
\textsf{AE}=autumn equinox, \textsf{SE}=spring equinox}\label{f.circle}
\end{center}
\end{minipage}
%%%%%%%%%%%%%%
\begin{minipage}{.48\textwidth}
...
% Draw the orbit of the Earth and the celestial sphere
\coordinate (S) at (0,0) node[above right,xshift=-2pt,yshift=-2pt] {$S$};
\draw[name path=Eorbit] (S) circle[radius=3cm];
\draw[name path=Csphere] (S) circle[radius=5cm];
\coordinate (O) at (-.5,.8);
\node[below left,yshift=-2pt] at (O) {$O$};
...
\draw[red,thick] (SE) -- (O) -- (SS);
\draw[blue,thick] ($(SS)+(-2pt,0)$) -- ($(O)+(-2pt,0)$) -- (AE);
\draw[purple,thick] ($(AE)+(0,-2pt)$) -- ($(O)+(0,-2pt)$) -- (WS);
\draw[green!70!black,thick] ($(WS)+(2pt,0)$) -- 
  ($(O)+(2pt,-2pt)$) -- ($(SE)+(0,-2pt)$);

% Names of the seasons
\node at (45:1.8) {\textsf{spring}};
\node[yshift=-2pt] at (140:2.1) {\textsf{summer}};
\node at (-135:2) {\textsf{autumn}};
\node at (-45:1.8) {\textsf{winter}};

\end{tikzpicture}
\caption{The lengths of the seasons\\are not equal\\%
\textsf{AE}=autumn equinox, \textsf{SE}=spring equinox}\label{f.seasons}
\end{center}
\end{minipage}
\end{figure}
...
%\begin{center}
%\begin{tikzpicture}
%\clip[draw] (-6,-3.5) rectangle +(12,7);
%
%% Draw the orbit of the Earth and the celestial sphere
%\coordinate (S) at (0,0) node[above right,xshift=-2pt,yshift=-2pt] {$O,S$};
...
\clip[draw] (-6.8,-2) rectangle (6.8,5.5);
...
\draw[very thick,dotted,blue] (B) -- 
  node[below,black] {$b$} (B -| BP) coordinate (BRA) -- (O);
\draw[very thick,dotted,red] (A) -- 
  node[left,black] {$a$} (A |- AP) coordinate (ARA) -- (O);
...
\node[below right,xshift=16pt,yshift=0pt] at (O) {$\alpha$};
...
Let us express the arcs $\widehat{AC}$ and $\widehat{AB}$ in terms of the arcs that comprise them. Since $AC\parallel A'C'$, $\widehat{AA'}=\widehat{C'C}$. Compute $\widehat{AA'}$.
\begin{eqn}
\widehat{AC} &=& \widehat{AA'} + \widehat{A'C'} + \widehat{C'C}= 2\widehat{AA'} + \pi\\[2pt]
...
\begin{eqn}
\widehat{AB} &=& \widehat{AA'} + \widehat{A'B'} - \widehat{BB'}\\[2pt]
...
\begin{eqn}
\sin \alpha &=& \frac{a}{r} \approx \alpha\\[2pt]
a &\approx& r\alpha = r\widehat{AA'} = 0.0298r\\[2pt]
\sin \beta &=& \frac{b}{r} \approx \beta\\[2pt]
...
\clip[draw] (-5.8,-1.8) rectangle +(8,7.2);
...
\draw[very thick,dotted,blue] (O) -- (B) -- 
  node[below,black] {$b$} (B -| BP) coordinate (BRA) -- cycle;
\draw[very thick,dotted,red] (O) -- (A) --
  node[left,black] {$a$} (A |- AP) coordinate (ARA) -- cycle;
