...
\vertexsm{V};
\draw (Q) -- (X) node[above left,xshift=-2pt,yshift=-4pt] {$X$} -- (V); \coordinate (VV) at ($(P)+(25pt,-6pt)$);
\vertexsm{VV};
\node[right] at (VV) {$=V$};
...
\node[below left,xshift=2pt,yshift=-2pt] at (O) {$C$};
\node[below] at (F1) {$S$};
\node[below] at (F2) {$H$};
...
\vertexsm{V};
\coordinate (VV) at ($(P)+(25pt,-6pt)$);
\vertexsm{VV};
\node[right] at (VV) {$=V$};

\path (P) -- ($(P)!2!(O)$) coordinate (G);
\draw[->,thick,dashed] (O) -- ($(O)!.25!(G)$) node[left,yshift=-2pt] {$G$};

% Draw T
\path (Q) -- ($(F1)!(Q)!(P)$) coordinate (T);
\node[above left,yshift=-3pt] at (T) {$T$};
\draw[rotate=13] ($(T)+(0pt,1pt)$) rectangle +(6pt,6pt);
...
\draw[rotate=33] (F) rectangle +(6pt,6pt);
...
% Label angles
\node[below right,xshift=6pt,yshift=8pt]   at (Q)  {\sm{\beta}};
\node[left,xshift=-2pt,yshift=2pt] at (P)  {\sm{\beta}};
\node[right,xshift=2pt,yshift=-3pt] at (E)  {\sm{\beta}};
...
(a) $X\rightarrow V$ so that $QX\rightarrow QV$, (b)
$V\rightarrow P$ so that $CV\rightarrow CP$, and (c) $GV\rightarrow 2CP$.
Substituting into Equation~\ref{eqn.three-multiplications} and using  Theorem~\ref{thm.ellipse-lr} gives
\[
