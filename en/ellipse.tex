...
\begin{proof}
\begin{enumerate}
\item $\triangle F_1CO$ is congruent to $\triangle F_2CO$ by side-angle-side ($F_1O=F_2O$, $CO$ is a common side, $\angle COF_i$ are right angles),  so $F_1C=F_2C$. Since $C$ is on the ellipse, $F_1C+F_2C=2a$ and $F_1C=F_2C=a$ follows. 
\item Since $A$ is on the ellipse,
\[
2a = AF_1+AF_2 = (AO-c)+(AO+c)=2AO\,,
\]
so $AO=a$. $OB=a=AO$ follows by symmetry.
\item $CO=OD$ follows since $\triangle F_1CO$ is congruent to $\triangle F_1DO$.\hqed
\end{enumerate}
\end{proof}
...
Consider a circle of radius $a$ with the same center as an ellipse (Figure~\ref{f.ellipse-circle}). Construct a vertical line at an arbitrary point $X=(x,0)$ on the $x$-axis, and let its intersections with the ellipse and the circle be $P_e=(x,y_e)$ and $P_c=(x,y_c)$, respectively.
...
y_e^2&=& b^2\left(1-\frac{x^2}{a^2}\right)=
\frac{b^2}{a^2}(a^2-x^2)\label{eq.point-on-ellipse}\\[4pt]
x^2+y^2_c &=& a^2\nonumber\\[4pt]
...
\draw[name path=minor] (O) -- node[left,yshift=12pt] {$a$} (T);
...
\begin{eqn}
L_1&=&\left(c,\frac{b}{a}\sqrt{a^2-c^2}\right)= \left(c,\frac{b^2}{a}\right)\\[4pt]
L&=&L_1L_2=\displaystyle\frac{2b^2}{a}\,.
\end{eqn}\hqed
\end{proof}
...
\sin^2 \theta &=& \frac{1-\cos 2\theta}{2}\\[4pt]
...
\begin{eqn}
A_c &=& 2\int_{-a}^{a} \sqrt{a^2-x^2} \;dx\\[4pt]
&=& 2\int_{-\pi}^{0} \sqrt{a^2-(a \cos \theta)^2} \;\;d(a\cos\theta)\\[4pt]
...
Let $PU$ be the extension of $SP$, the line from the focus $S$ to a point $P$  on the ellipse, such that $SU=AA'=2a$, and let $HP$ be the line from the other focus to $P$ (Figure~\ref{f.tangent-angles}). Let $RQ$ be the bisector of $\angle HPU$. Then $RQ$ is the tangent to the ellipse at $P$ and $\angle RPS = \angle QPH$.
\end{theorem}
...
All parallelograms formed by tangents to the intersections of conjugate diameters with the ellipse are equal (Figure~\ref{f.conj-ratios}).
\end{theorem}
...
\draw[thick,dashed,blue] (D) -- ($(L)!(D)!(R)$) node[below] {$D'$};
\draw[thick,dashed,blue] (P) -- ($(L)!(P)!(R)$) node[below] {$P'$};
...
&=& \frac{y_p+y_d}{2} (x_p-x_d) - \frac{1}{2}x_dy_d - \frac{1}{2}x_py_p\\[6pt]
...
Let $P=(x,y)$ be a point on an ellipse (not on the major axis) and drop a perpendicular $PV$ from $P$ to the major axis (Figure~\ref{f.ellipse-conj-diam-proof}). Then
\[
\frac{A'V\cdot AV}{PV^2} = \frac{a^2}{b^2}\,.
\]
\end{theorem}
\begin{proof}
Using the formula for a point on the ellipse (Equation~\ref{eq.point-on-ellipse}) we get
\begin{eqn}
...
\subsection{Ratios on conjugate diameters}
\begin{theorem}(Figure~\ref{f.conj-ratios})\label{thm.conj-diag}
\begin{eqn}
\frac{GV\cdot PV}{QV^2} &=& \frac{CP^2}{CD^2}\\
PV &=& \frac{QV^2\cdot CP^2}{GV \cdot PV \cdot CD^2}\,.
\end{eqn}
\end{theorem}
...
\]
The ellipses are different so $a'\neq a$ and $b'\neq b$, but by construction $a'=CP$ and $b'=CD$ so the theorem follows.\hqed
\end{proof}
