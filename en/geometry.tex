...
\begin{theorem}[Prop.~II of Chapter~I]\label{thm.bisect}
...
\begin{theorem}[Prop.~I]\label{thm.right-angle}
...
\begin{theorem}[Prop.~VIII]\label{thm.ellipse-b-over-a-besant}
...
\begin{theorem}[Prop.~XIV]\label{thm.cnntacac}
...
\begin{theorem}[Prop.~XVII]\label{thm.parallelogram1}
$PF\cdot PG = BC^2$.
\end{theorem}
...
\begin{theorem}[Prop.~XXII]\label{thm.cmpnacbc}
\[
...
\node[below left] at (Left) {$A'$};
\node[below right] at (Right) {$A$};
\node[below left,xshift=3pt,yshift=-2pt] at (O) {$C$};
...
\begin{theorem}[Prop.~XXVI](Theorem~\ref{thm.conj-diam-para})\label{thm.area-parallelogram}
...
\section{Equal angles between the tangents the lines to the foci}

The following theorem is used in Section~\ref{s.marden}.

\begin{theorem}[Prop. XIII]\label{thm.marden-tangent} If two tangents are constructed to an ellipse from an external point, the angle between them and lines to the foci are equal (Figure~\ref{f.tangent-equal-angles}).
\end{theorem}
...
% Conjugate diameters from D through C
\path[name path=cd] (O) -- +({\angle+90}:{1.5*\a} and {1.5*\b});
\path [name intersections = {of = cd and ellipse, by = {PP} }];
\node[above] at (PP) {$P'$};
% Draw tangent at D
\draw[name path global=t3] 
  (PP) -- ++ ({-\a*sin(90+\angle)},{\b*cos(90+\angle)})
  (PP) -- ++ ({\a*sin(90+\angle)},{-\b*cos(90+\angle)}) coordinate (Q);
\node[above] at (Q) {$Q$};
\draw (F1) -- ($(PP)!(F1)!(Q)$) coordinate (ZP);
\node[above] at (ZP) {$Z'$};
\draw (F1) -- ($(P)!(F1)!(Q)$) coordinate (YP);
\node[right] at (YP) {$Y'$};
\draw (F2) -- ($(PP)!(F2)!(Q)$) coordinate (Z);
\node[above] at (Z) {$Z$};
\draw (F2) -- ($(P)!(F2)!(Q)$) coordinate (Y);
\node[right] at (Y) {$Y$};

\draw (Y) -- (Z);
\draw (YP) -- (ZP);

\draw[rotate=110] (Y) rectangle +(6pt,6pt);
\draw[rotate=110] (YP) rectangle +(6pt,6pt);
\draw[rotate=-78] (Z) rectangle +(6pt,6pt);
\draw[rotate=-78] (ZP) rectangle +(6pt,6pt);

\draw[thick,red]  (F2) -- (Q) -- (P);
\draw[thick,blue]  (F1) -- (Q) -- (PP);

\coordinate (X) at ($(F1)!8mm!(YP)$);
\coordinate (XX) at ($(F2)!8mm!(Y)$);
\draw (X) arc[start angle = 30, end angle = {105},  radius = 8mm];
\draw (XX) arc[start angle = 30, end angle = {105},  radius = 8mm];
\node[above left,xshift=-4pt,yshift=8pt] at (X) {$\alpha$};
\node[above left,xshift=-4pt,yshift=8pt] at (XX) {$\alpha$};

\node[below left,xshift=-25pt,yshift=-5pt] at (Q) {$\beta$};
\node[below,xshift=3pt,yshift=-16pt] at (Q) {$\beta$};

\end{tikzpicture}
\caption{Equal angles between tangents and lines to the foci}\label{f.tangent-equal-angles}
\end{center}
\end{figure}
