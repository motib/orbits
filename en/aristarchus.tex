...
The angle that Eratosthenes measured was 
\[
...
\label{eq.re}
\end{equation}%
This value is close to the modern measurement of $6370$ km.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\newpage

\section{Aristarchus's measurements of radii and distances}

Using $r_e$ Aristarchus was able to measure and compute the following values.
\begin{itemize}
...
\draw (M) rectangle +(6pt,6pt);
\vertexsm{M};

% Draw the sun
\coordinate (S) at (10,0);
\node at ($(S)+(0,2.2)$) {\textsf{Sun}};
...
% Draw the earth
\coordinate (E) at (-1,3);
\node at ($(E)+(1.8,.2)$) {\textsf{Earth}};
\draw (E) circle[radius=1cm];
...
%  Equation~\ref{eq.dm-ds} and Figure~\ref{f.solar-eclipse},
\begin{eqnlabels}
...
\clip (-4,-1.4) rectangle +(14.2,2.8);

% Draw the sun and two radii
\coordinate (S) at (8.5,0);
\node[below] at ($(S)+(0,-1.5)$) {\textsf{Sun}};
\draw (S) circle[radius=1.25cm];
...
%% Draw the moon black
\coordinate (M) at (-2,0);
\draw[fill=gray!90!black] (M) circle[radius=.3cm];
...
\frac{r_s-r_e}{r_e-2r_m}&=&\frac{d_s}{d_m}=\frac{r_s}{r_m}\\[2pt]
r_sr_e+r_mr_e&=&3r_sr_m\\[2pt]
19r_mr_e + r_mr_e &=& 3\cdot 19 r_m^2\\[2pt]
r_m&=&\frac{20}{57}r_e\,.
\end{eqn}
...
% Draw Sun
\coordinate (S) at (10,0);
\draw (S) circle[radius=2cm];
\vertexsm{S};
\node[below] at ($(S)+(0,-2)$) {\textsf{Sun}};

% Draw earth
\coordinate (E) at (4,0);
\draw (E) circle[radius=1.25cm];
\vertexsm{E};
\node[below] at ($(E)+(0,-2)$) {\textsf{Earth}};

% Draw moon
\coordinate (M) at (1,0);
\draw (M) circle[radius=.45cm];
\vertexsm{M};
